...
$stmt = $conn->query("SELECT * FROM career ORDER BY id DESC");
...
if (isset($_GET['download']) && $_GET['download'] === 'csv') {
    ...
    fputcsv($csv, array_keys($rows[0] ?? ['id', 'first_name', 'last_name', 'email', 'role', 'phone_number', 'attachment', 'experience']), ',', '"', '');
    foreach ($rows as $row) {
...
<head>
    ...
    <title>Career Applications - Prismabit</title>
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="sidebar w-64 flex-shrink-0 flex flex-col fixed h-full">
            ...
            <nav class="flex-1 p-4 space-y-1">
                ...
                <a href="careers.php" class="sidebar-link active flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    Careers
                </a>
                <a href="audits.php" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium">
        ...
        <div class="flex-1 ml-64">
            <header class="topbar sticky top-0 z-30 px-8 py-4 flex items-center justify-between">
                <h2 class="text-lg font-semibold">Career Applications</h2>
            ...
            <div class="p-8">
                <div class="data-card rounded-xl p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="font-semibold">All Career Applications (<?= count($rows) ?>)</h3>
                        <a href="?download=csv" class="text-sm text-[#007BFF] hover:underline">Export CSV</a>
                    </div>

                    <?php if (empty($rows)): ?>
                        <p class="text-gray-500 text-sm py-10 text-center">No career applications yet.</p>
                    <?php else: ?>
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody>
                                ...
                                <tr class="table-row border-b border-white/5">
                                    ...
                                    <td class="px-4 py-3"><?= htmlspecialchars($row['experience']) ?></td>
                                    <td class="px-4 py-3">
                                        <?php if (!empty($row['attachment'])): ?>
                                            <a href="../<?= htmlspecialchars($row['attachment']) ?>" target="_blank" class="text-[#007BFF] hover:underline">View CV</a>
                                        <?php else: ?>
                                            <span class="text-gray-600">No CV</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
