...
if (isset($_GET['download']) && $_GET['download'] === 'csv') {
    ...
    fputcsv($csv, array_keys($rows[0] ?? ['id', 'first_name', 'last_name', 'email', 'message']), ',', '"', '');
    foreach ($rows as $row) {
...
<head>
    ...
    <title>Contact Submissions - Prismabit</title>
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="sidebar w-64 flex-shrink-0 flex flex-col fixed h-full">
            ...
            <nav class="flex-1 p-4 space-y-1">
                ...
                <a href="contacts.php" class="sidebar-link active flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium">
            ...
            <div class="p-8">
                <div class="data-card rounded-xl p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="font-semibold">All Contact Messages (<?= count($rows) ?>)</h3>
                        <a href="?download=csv" class="text-sm text-[#007BFF] hover:underline">Export CSV</a>
                    </div>

                    <?php if (empty($rows)): ?>
                        <p class="text-gray-500 text-sm py-10 text-center">No contact submissions yet.</p>
                    <?php else: ?>
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody>
                                ...
                                <tr class="table-row border-b border-white/5">
                                    ...
                                    <td class="px-4 py-3"><a href="mailto:<?= htmlspecialchars($row['email']) ?>" class="text-[#007BFF] hover:underline"><?= htmlspecialchars($row['email']) ?></a></td>
